...
CopyFile.prototype.doCopy = function(src, dest){
    var self = this;
    var filePath = self.filePaths[dest][self.index[dest]];
    ...
    if (!filePath) {// 当前目录下已copy完成
        if (process.cwd() === dest) {// 当前目录未根目录，表示copy完成
            ...
            self.doCopy(parentDirSrc, parentDirDest);
        }
    } else {
        ...
        if(st.isFile()){// 判断是否为文件
            ...
            if (exists) {
                prompt(color.cyan(FLAG + childDirDest.match(BASE_REGEXP)[0] + ' already exists, is overwrite(Y/N)?'), function (val) {
                    if (val.toUpperCase() === 'Y') {
                        self.createFile(childDirSrc, childDirDest);
                    } else {
                        self.index[dest] += 1;
                        self.doCopy(src, dest);
                    }
                });
            } else {
                self.createFile(childDirSrc, childDirDest);
            ...
            self.createChildDir(childDirSrc, childDirDest);
        }
    }
};

CopyFile.prototype.createFile = function(src, dest){
    var self = this;
    // 创建读取流
...
CopyFile.prototype.createChildDir = function(src, dest){
    var self = this;
    var parentDirSrc = path.resolve(src, '../');
    var parentDirDest = path.resolve(dest, '../');
    // 读取目录是否存在
